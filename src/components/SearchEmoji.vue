...
<template>
  <div class="space-y-6">
    ...
    <div class="max-w-2xl mx-auto">
      <div class="relative">
        <input v-model="searchQuery" type="text" placeholder="Search emojis by name (e.g., heart, smile, face)..."
          class="w-full px-4 py-3 pr-12 border-2 border-gray-300 rounded-lg focus:border-teal-500 focus:outline-none text-lg" />
        <svg class="absolute right-4 top-1/2 transform -translate-y-1/2 w-6 h-6 text-gray-400" fill="none"
    ...
    <div v-if="searchQuery.trim()">
      ...
      <div v-if="searchResults.length > 0"
        ...
        <button v-for="emoji in searchResults" :key="emoji.unicode + '-' + emoji.shortname" @click="selectEmoji(emoji)"
